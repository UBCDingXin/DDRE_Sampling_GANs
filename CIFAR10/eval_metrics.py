...
def FID_RAW(PreNetFID, IMGSr, IMGSg, batch_size = 500, NGPU=2, resize = None):
...
#    PreNetFID.load_state_dict(checkpoint_PreNet['net_state_dict'])
    ...
    with torch.no_grad():
        test_img = torch.from_numpy(IMGSr[0].reshape((1,3,32,32))).type(torch.float).cuda()
...
##test
#IMGSr = np.random.rand(50000,3,32,32)
#IMGSg = np.random.rand(10000,3,32,32)
#print(FID_RAW(PreNet, checkpoint_PreNet, IMGSr, IMGSg, NGPU=2))
...
#    PreNetIS.load_state_dict(checkpoint_PreNet['net_state_dict'])
    ...
    with torch.no_grad():
        test_img = torch.from_numpy(IMGSg[0].reshape((1,3,32,32))).type(torch.float).cuda()
        if resize is not None:
            test_img = nn.functional.interpolate(test_img, size = resize, scale_factor=None, mode='bilinear', align_corners=False)
