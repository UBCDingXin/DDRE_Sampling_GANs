...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, depth=1):
        super(BasicBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        # self.bn1 = nn.BatchNorm2d(planes)
        self.bn1 = nn.GroupNorm(4*depth, planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        # self.bn2 = nn.BatchNorm2d(planes)
        self.bn2 = nn.GroupNorm(4*depth, planes)
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                ...
                nn.GroupNorm(4*depth, self.expansion*planes)
            )
...
class DR_ResNet(nn.Module):
    def __init__(self, block, num_blocks, ngpu=2, p_dropout=0.4, fc_layers=False, final_ActFn="Softplus"):
        ...
        convs = [nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False),
                 ...
                 # nn.GroupNorm(4, 64),
                 nn.ReLU()]
        convs += self._make_layer(block, 64, num_blocks[0], stride=1, depth=1)
        convs += self._make_layer(block, 128, num_blocks[1], stride=2, depth=2)
        convs += self._make_layer(block, 256, num_blocks[2], stride=2, depth=3)
        convs += self._make_layer(block, 512, num_blocks[3], stride=2, depth=4)
    ...
    def _make_layer(self, block, planes, num_blocks, stride, depth=1):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, depth=1))
